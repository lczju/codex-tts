...
搭建一套面向多语言、可重复、便于试听对比和对外展示的 TTS 实验流程，用来对现有主流方法进行复现、微调与测评，并为后续可能加入的 `VALL-E` 类方案预留位置。
...
- 用静态网页展示测试文本、音频、备注、主观评分和对外可视化结果

当前阶段约束：

- 项目整体定位是多语言 TTS 实验与展示
- 当前第一阶段仍以中文数据、中文测试文本和中文 baseline 为主
- 多语言扩展会在现有评测结构稳定后逐步补齐
...
- 先跑通中文主线最小闭环，再扩语言、扩模型、扩流程、扩工程复杂度
...
- 在云端 GPU 上对当前 `15` 条测试文本运行一轮 `CosyVoice` pretrained inference
- 下载生成的 wav
...
3. 补实验日志模板
4. 启动 `VITS` baseline
5. 补英文或更多语言的测试样例与展示入口
6. 评估是否需要接入更多模型或自动化评测
