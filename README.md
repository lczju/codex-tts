...
一个面向多语言 TTS 的实验与展示仓库，用来对现有主流语音合成方法进行复现、微调、测评，并将结果整理成可试听、可记录、可持续扩展、可直接对外展示的可视化工作台。

当前重点围绕这些事情展开：

- 使用统一测试文本对比 `CosyVoice`、`VITS` 等方法，并为后续接入 `VALL-E` 类方案预留结构
- 在云端 GPU 机器上做推理或小规模 fine-tuning
- 在本地 Windows 环境中整理数据、管理结果、试听对比和记录评分
- 用 `web/` 下的静态页面承载测试文本、结果音频、主观评测和对外展示内容

当前阶段说明：

- 项目整体目标是多语言 TTS 复现、微调与测评
- 当前已落地的数据、测试文本和 baseline 路线仍以中文场景为主
- 后续会在现有结构上逐步扩展英文及更多语言实验

## 当前进度

截至 `2026-06-22`，项目已经从“纯文档规划”进入“展示工作台 + 中文主线数据准备已跑通”的阶段。
...
5. 在本地页面里统一试听、打分、对比，并作为对外可视化展示入口
...
5. 在中文主线稳定后，补充英文或更多语言的测试集、数据集与展示样例
6. 最后再考虑接入更多模型或自动化评测
